...
//     numerator() int
//     denominator() int
//     equal(s Rational) bool
//     invert() Rational
//     add(s Rational) Rational
//     multiply(s Rational) Rational
//     divide(s Rational) Rational
//     ToLowestTerms() Rational
// }
...
func ToLowestTerms(r Rational) Rational {
    a, b := Split(r)
    d := Gcd(a, b)
    if d == 1 || d == -1 {
        return r
...
func main() {
    ...
    for i := 1; i <= N; i++ {
        ...
        if err != nil {
            ...
            // fmt.Printf("%v\n", r)
            total = ToLowestTerms(Add(total, r))
