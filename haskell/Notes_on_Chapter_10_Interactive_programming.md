...
mix together. It is left up to the programmer to distinguish between them in
Most programming languages do nothing, and allow pure and impure functions to
every case.
...
asks you to imagine an interactive Haskell function, e.g. maybe a chatbot that
the user interacts with through text. In Haskell, you *can't* implement an
interesting chatbot with the signature `String -> String`. Since the function is
pure, the chatbot will return the same result every time you give it the same
input.

But what you can do is imagine that the chatbot function has the type `World ->
(String, World)`. The `World` type is a made-up type (just for this example)
that represents the entire state of the world that the chatbot knows about. This
includes many values, such as a list of all previous inputs to the chatbot, the
current date/time, a knowledge base of rules for answering questions, and so on.
The input to the chatbot is a state of the `World`, and the output is the
chatbot's reply of type `String`, plus a new `World` state that describes the
world after the chatbot has replied. Instead of *modifying* the world, the
chatbot *makes a new copy* of the world. A pure function can't *modify* the
world, but in can *copy* it. 

You can think of this pure chatbot function as having the type signature `World
-> (String, World)`. `World` is not a real type, and instead Haskell uses the
type `IO String`, which you could imagine as being defined like this:
...
world as input, and returning a value plus an updated state of the world as
output. The side-effects of the function are recorded in this changed world.
Intuitively, the function *doesn't change* the world, but instead returns a
brand new modified copy of the world.

Expressions of type `IO a` are called **actions**. For example, `IO String` is
the type of an action that returns an `String`, and `IO Int` is the type of an
...
performs of printing the character on the screen.
...
from the `IO a`. Once a value is in `IO a`, it is stuck there forever.
